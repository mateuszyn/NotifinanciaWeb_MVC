    ...
    render(assets, user) {
        ...
        app.innerHTML = `
            ...
            <div class="container mt-4 mb-5 pb-5">
                ${PortfolioSummaryView.render(portfolioSummary)}
                <div class="row" id="asset-list">${AssetCardView.renderList(assets, user)}</div>
                ${PromptView.render(assets, user)}
            </div>
            ${user.isGuest ? '' : `<div id="add-asset-drawer" class="bottom-drawer ${isDrawerOpen ? '' : 'collapsed'}"><div class="drawer-header" id="drawer-toggle"><div class="drag-handle"></div><button class="btn btn-success w-100 fw-bold py-2 mt-3 fake-add-btn">+ NOVO ATIVO</button></div><div class="drawer-content" id="form-container"></div></div>`}
