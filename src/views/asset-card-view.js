    ...
    render(asset, user) {
        ...
        const dyInfoTooltip = `
            <details class="dy-info-wrapper">
                ...
                <div class="dy-info-popover text-start">
        ...
        const dividendContent = asset.dataError ? `<div class="text-center p-2"><p class="small text-warning mb-2">Não foi possível carregar os dados.</p><button class="btn btn-sm btn-outline-warning btn-retry-asset" data-ticker="${safeTicker}"><i class="bi bi-arrow-clockwise"></i> Tentar novamente</button></div>` : yieldPct === 0 ? `<div class="text-center p-2"><p class="small text-warning mb-2">Não foi possível calcular dividendos.</p><button class="btn btn-sm btn-outline-warning" onclick="window.location.reload()"><i class="bi bi-arrow-clockwise"></i> Recarregar</button></div>` : `<div class="d-flex justify-content-around"><div><p class="small text-secondary mb-0">Renda Mensal</p><p class="fw-bold mb-0 text-white">R$ ${divMensal.toFixed(2)}</p><p class="small mt-1 mb-0" style="color: #8fe3a7;">Gera ${cotasCompradasPorMes.toFixed(2)} cota(s) / mês</p></div><div><p class="small text-secondary mb-0">Renda Anual</p><p class="fw-bold mb-0 text-white">R$ ${divAnual.toFixed(2)}</p><p class="small mt-1 mb-0" style="color: #8fe3a7;">Gera ${cotasCompradasPorAno.toFixed(2)} cota(s) / ano</p></div></div>`;
            ...
            <div class="col-12 col-md-6 col-lg-4 mb-4">
                <div class="asset-card ${borderClass}">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <div class="d-flex gap-3 align-items-center">${actions}</div>
                    </div>
                    <div class="row mb-3">
                        <div class="col-12">
                            <p class="price-value mb-0">Total: R$ ${(quantity * currentPrice).toFixed(2)}</p>
                            <p class="small text-secondary fw-bold mb-1">QTD: ${quantity}</p>
                            <p class="small mb-0" style="font-size: 0.72rem; color: ${snowballColor};">${snowballMessage}</p>
                        </div>
                    </div>
                    <div class="row border-top border-bottom border-secondary py-2 mb-3">
                        ...
                        <div class="col-6 ps-3">
                            ...
                            <p class="price-value d-flex align-items-center gap-1">R$ ${currentPrice.toFixed(2)} <span class="${dailyTextClass} small">(${dailyChange >= 0 ? '+' : ''}${dailyChange.toFixed(2)}%)</span></p>
                        </div>
                    </div>
                    <div class="bg-dark rounded p-2 text-center border border-secondary mb-3 mx-0 dividend-box-container">
                        ${yieldPct > 0 ? dyInfoTooltip : ''}
                        <div class="mb-2">
